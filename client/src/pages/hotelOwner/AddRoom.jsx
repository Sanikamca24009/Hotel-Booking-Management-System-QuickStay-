...
const AddRoom = () => {
    ...
    const onSubmitHandler = async (e) => {
        ...
        try {
            ...
            // Converting Amenities to Array & keeping only enabled Amenities that are valid for the selected room type
            const roomTypeAmenities = {
                'Single Bed': ['Free WiFi', 'Room Service'],
                'Double Bed': ['Free WiFi', 'Free Breakfast', 'Room Service'],
                'Luxury Room': ['Free WiFi', 'Free Breakfast', 'Room Service', 'Mountain View', 'Pool Access'],
                'Family Suite': ['Free WiFi', 'Free Breakfast', 'Room Service', 'Pool Access']
            };
            const validAmenities = inputs.roomType ? roomTypeAmenities[inputs.roomType] : Object.keys(inputs.amenities);
            const amenities = Object.keys(inputs.amenities).filter(key => inputs.amenities[key] && validAmenities.includes(key));
    ...
    return (
        <form onSubmit={onSubmitHandler}>
            ...
            <p className='text-gray-800 mt-4'>Amenities {!inputs.roomType && <span className='text-xs text-gray-400'>(Select a room type first to view options)</span>}</p>
            <div className='flex flex-col flex-wrap mt-1 text-gray-400 max-w-sm'>
                {(() => {
                    ...
                    const availableAmenities = inputs.roomType ? roomTypeAmenities[inputs.roomType] : Object.keys(inputs.amenities);
                    
                    return availableAmenities.map((amenity, index) => (
                        <div key={index}>
                            <input type='checkbox' id={`amenities${index + 1}`} checked={inputs.amenities[amenity] || false}
